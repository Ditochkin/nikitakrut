...
private:
	...
	vector<rgb>centr;// Вектор центров
	void identify_centers();// Выбор начальных центров
...
public:
	K_means() : q_klaster(0), k_pixcel(0) {};// Конструктор по умолчанию
	K_means(int n, rgb* mas, int n_klaster);// Конструктор из массива
	K_means(int n_klaster, istream& os);// Конструктор из файла
